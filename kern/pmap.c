...
//		else if (n==0)
	...
	//		mon_backtrace();
			panic("out of memore mathafaka at :");
		else
...
//	size_t HOLE_PG= (IOPHYSMEM) ;
	
	//RETURN FIRST FREE BYTE
	char* end_of_kernel = boot_alloc(0);
	
//	pages[0].pp_ref = 1;
	for (i=1 ; i < npages; i++) {
		if(page2pa(&pages[i])>= IOPHYSMEM &&(char*) page2kva(&pages[i]) < end_of_kernel){
			pages[i].pp_ref = 1;
...
page_alloc(int alloc_flags)
{
	...
	if(alloc_flags & ALLOC_ZERO){
		uint32_t* _VAallocPage = page2kva(alloc_page);	
		memset(_VAallocPage, '\0',PGSIZE);
	}
	return alloc_page;
}
...
page_free(struct PageInfo *pp)
{
	...
	if(pp->pp_ref || pp->pp_link )
		panic("page_free_list is not empty");
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	for (i = 0; i < size/PGSIZE; ++i, va += PGSIZE, pa += PGSIZE) {
		...
		if (!pte) panic("boot_map_region pte == null");
		*pte = pa | perm | PTE_P;
	}
}
...
{
	// Fill this function in
	pte_t *pageTable =   pgdir_walk(pgdir,va, false);
	if (!pageTable|| !(*pageTable& PTE_P))
		 return NULL;
	if (pte_store)
		*pte_store =pageTable ;	
